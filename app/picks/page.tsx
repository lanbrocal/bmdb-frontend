  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white p-8">
      <header className="mb-12 text-center">
        ...
        <h1 className="text-5xl font-extrabold mb-2 text-yellow-500 italic uppercase tracking-tighter">
          The Vault
        </h1>
        <p className="text-gray-400 font-mono text-sm">Etrain's Personal Hall of Infamy</p>
      </header>

      {isLoading ? (
        <div className="text-center py-20 animate-pulse text-yellow-500 font-bold">Opening the Vault...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
          {picks.length > 0 ? (
            picks.map((movie) => (
              <div key={movie.tmdb_id} className="bg-gray-900 rounded-xl overflow-hidden shadow-2xl border border-yellow-900/30 hover:border-yellow-500 transition-all group relative">
                <div className="relative aspect-[2/3] overflow-hidden bg-gray-800">
                  <div className="absolute top-2 left-2 z-10 bg-yellow-500 text-black text-[10px] font-bold px-2 py-1 rounded shadow-lg flex items-center gap-1">
            ...
            <div className="col-span-full text-center py-20">
              <p className="text-gray-500 italic">The Vault is currently empty. Tag some movies in the database!</p>
            </div>
          )}
        </div>
      )}
